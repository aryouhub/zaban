...
export default function FlashCard({ words, onBack }: FlashCardProps) {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      ...
      {/* Flash Card */}
      <div
        className={`flip-card w-full h-72 md:h-80 cursor-pointer mb-6 ${isFlipped ? 'flipped' : ''}`}
        onClick={handleFlip}
      >
        <div className="flip-card-inner relative w-full h-full">
          ...
          <div className="flip-card-front absolute inset-0 glass-strong rounded-3xl flex flex-col items-center justify-center p-6">
            ...
            <div className="text-gray-400 text-sm mb-4" dir="ltr">
          ...
          <div className="flip-card-back absolute inset-0 glass-strong rounded-3xl flex flex-col items-center justify-center p-6">
            ...
            <button
              ...
              className="text-sm text-indigo-400 hover:text-indigo-300 transition-colors mb-2"
            >
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        ...
        <div className="flex gap-3 flex-1 justify-center">
          <button
            ...
            className="flex-1 max-w-[140px] py-3 rounded-xl bg-red-500/20 border border-red-400/30 text-red-300 hover:bg-red-500/30 transition-all font-medium text-sm"
          >
            ✗ بلد نبودم
          </button>
          <button
            onClick={handleKnown}
            className="flex-1 max-w-[140px] py-3 rounded-xl bg-green-500/20 border border-green-400/30 text-green-300 hover:bg-green-500/30 transition-all font-medium text-sm"
          >
            ✓ بلد بودم
          </button>
        </div>
      ...
      <div className="text-center">
        <span className={`text-xs px-3 py-1 rounded-full ${
          currentWord.level === 'beginner' ? 'bg-green-500/20 text-green-400' :
          currentWord.level === 'intermediate' ? 'bg-yellow-500/20 text-yellow-400' :
          'bg-red-500/20 text-red-400'
        }`}>
          {currentWord.level === 'beginner' ? '🌱 مبتدی' :
           currentWord.level === 'intermediate' ? '🌿 متوسط' : '🌳 پیشرفته'}
        </span>
      </div>
    </div>
  );
